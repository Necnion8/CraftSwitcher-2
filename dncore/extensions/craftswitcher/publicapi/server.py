...
class UvicornServer(object):
    ...
    async def start(self, app: "ASGIApplication | Callable | str", *, host="127.0.0.1", port=8000, path=""):
        if self._runner and not self._runner.done():
            raise RuntimeError("Already started server")

        config = uvicorn.Config(app, host, port, root_path=path, server_header=False)
